...
| :--- | :--- | :--- | :--- |
| OpenAI API | https://platform.openai.com/ | Pay-as-you-go | クレジットカード登録が必要 |
| Serper API | https://serper.dev/ | Free（2,500検索/月） | 無料枠で十分開始可能 |
| X Developer API | https://developer.x.com/ | Free Tier | 月500ポスト上限。申請に数日かかる場合あり |
...
| リスク | 発生確率 | 影響度 | 対策 |
| :--- | :--- | :--- | :--- |
| **投稿回数制限超過** | 低 | 中 | Free Tierの月500ポスト上限に対し、1日5ポスト×30日=150ポストで運用。上限の30%しか使わない設計。 |
...
| 項目 | 上限設定 | 監視方法 |
| :--- | :--- | :--- |
| OpenAI API | 月$30 | OpenAI管理画面のUsage Limits + スクリプト内カウンター |
| Serper API | 月10,000検索 | Serper管理画面 + スクリプト内カウンター |
| AI動画生成API | 月$50 | スクリプト内カウンター（`cost_tracker.json`） |
| **合計月間上限** | **$100（約15,000円）** | 上限超過時に自動停止 + マスター通知 |
...
| **APIコスト/月** | 約3,000円 | 約5,000円 | 約8,000円 |
